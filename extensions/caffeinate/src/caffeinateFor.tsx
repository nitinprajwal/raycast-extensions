...
export default async function Command(props: { arguments: Arguments }) {
  ...
  if (totalSeconds <= 0) {
    await showToast(Toast.Style.Failure, "Duration must be greater than zero");
    return;
  }

  const formattedTime = `${hours ? `${hours}h` : ""}${minutes ? `${minutes}m` : ""}${seconds ? `${seconds}s` : ""}`;

  await startCaffeinate({ status: true }, `☕ Caffeinating your Windows PC for ${formattedTime}`, {
