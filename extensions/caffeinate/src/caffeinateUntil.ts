...
export default async function Command(props: { arguments: Arguments }) {
  ...
  if (isCaffeinateRunning()) {
    await showToast(Toast.Style.Animated, "☕ Already caffeinated", "Stop current caffeination first");
    return;
  }
  ...
  if (!pattern.test(time)) {
    await showToast(Toast.Style.Failure, "Unrecognized time format", "Use formats like: 14:30, 2:30pm, 9am");
    return;
  }
  ...
  await startCaffeinate({ status: true }, `☕ Caffeinating your Windows PC until ${tomorrow}${formattedTime}`, {
